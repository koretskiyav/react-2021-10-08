...
const productsRestaurantsIdsSelector = createSelector(
  restaurantsListSelector,
  (restaurants) => {
    const productsRestaurantsIds = {};
    restaurants.map((restaurant) => {
      return restaurant.menu.map(productId => productsRestaurantsIds[productId] = restaurant.id);
    })
    return productsRestaurantsIds;
  }
);
...
export const orderProductsSelector = createSelector(
  orderSelector,
  productsSelector,
  productsRestaurantsIdsSelector,
  (order, products, productsRestaurantsIds) =>
    Object.keys(order)
      ...
      .map((product) => {
        return {
          ...
          restaurantId: productsRestaurantsIds[product.id]
        }
      })
);
